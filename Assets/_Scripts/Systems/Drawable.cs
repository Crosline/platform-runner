...
namespace FreeDraw {
    ...
    public class Drawable : MonoBehaviour {
        public static event Action<Color> OnDrawEvent;
        // PEN COLOUR
        ...
        public delegate void Brush_Function(RaycastHit hit);
        ...
        public Color Reset_Colour = new Color(255f, 255f, 255f, 0f);  // By default, reset the canvas to be transparent
        ...
        public void PenBrusHit(RaycastHit hit) {
            Vector2 pixel_pos = WorldToPixelCoordinates(hit);
        ...
        void Update() {
            if (GameManager.Instance.State != GameManager.GameState.Painting)
                return;
        ...
        public void MarkPixelsToColour(Vector2 center_pixel, int pen_thickness, Color color_of_pen) {
            ...
            for (int x = center_x - pen_thickness; x <= center_x + pen_thickness; x++) {
                ...
                for (int y = center_y - pen_thickness; y <= center_y + pen_thickness; y++) {
                    if (Mathf.Pow(x - center_x, 2) + Mathf.Pow(y - center_y, 2) <= Mathf.Pow(pen_thickness, 2))
        ...
        public Vector2 WorldToPixelCoordinates(RaycastHit hit) {
            // Change coordinates to local coordinates of this image
            Vector3 local_pos = transform.InverseTransformPoint(hit.point);

            // Change these to coordinates of pixels
            float pixelWidth = drawable_sprite.rect.width;
            float pixelHeight = drawable_sprite.rect.height;
            float unitsToPixels = pixelWidth / hit.collider.bounds.size.x * transform.localScale.x * 5f;
